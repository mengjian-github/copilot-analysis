...
const i = require(49674);
const o = require(28e3);
const s = require(46305);
const a = require(71017);
class Language {
  constructor(e, t, n) {
    this.languageId = e;
    this.isGuess = t;
...
class u extends LanguageDetection {
  constructor(e, t) {
    super();
    this.delegate = e;
    this.notebookDelegate = t;
    this.cache = new r.LRUCacheMap(100);
  }
  async detectLanguage(e) {
    const t = a.basename(e.fileName);
    return s.isNotebook(t) ? this.notebookDelegate.detectLanguage(e) : this.detectLanguageForRegularFile(t, e);
  }
  async detectLanguageForRegularFile(e, t) {
    let n = this.cache.get(e);
    if (n) {
      n = await this.delegate.detectLanguage(t);
      if (n.isGuess) {
        this.cache.set(e, n);
      }
    }
...
class p extends LanguageDetection {
  constructor() {
    super(...arguments);
    this.languageIdByExtensionTracker = new d();
  }
  async detectLanguage(e) {
    const t = a.basename(e.fileName);
    const n = a.extname(t).toLowerCase();
    const r = this.extensionWithoutTemplateLanguage(t, n);
    const i = this.detectLanguageId(t, r);
    return new Language(i.languageId, i.isGuess, this.computeFullyQualifiedExtension(n, r));
  }
  extensionWithoutTemplateLanguage(e, t) {
    if (o.knownTemplateLanguageExtensions.includes(t)) {
      const n = e.substring(0, e.lastIndexOf("."));
      const r = a.extname(n).toLowerCase();
